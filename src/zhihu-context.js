...
export function zhihuEchoMarkup(record){
  ...
  // A saved record may still point to an older question-only reference. Never
  // grant that record the evidence of the newly mapped answer or named author.
  const evidenceStatus=grounding?.status??(sources.every(item=>sourceEvidenceLevel(item)==='topic-only')?'topic-only':sources.some(item=>sourceEvidenceLevel(item)==='author-pending')?'author-pending':'excerpt');
  const title=evidenceStatus==='topic-only'?'知乎同题讨论':'知乎经验回声';
  const status=evidenceStatus==='topic-only'?'经验待补核':evidenceStatus==='author-pending'?'作者待核对':'相关讨论';
  ...
  const references=sources.map(item=>`<p><a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(item.title)} ↗</a><br><small>${esc(item.sourceKind==='question'?'问题页 · '+item.author:'知乎作者 · '+item.author)} · ${esc(item.evidence??'2026-09-08 CLI 搜索片段，未读取完整原文')}</small></p>${item.excerpt?`<p class="zhihu-verified-excerpt"><small>本轮核对片段 · ${esc(item.checkedAt)}</small><br>“${esc(item.excerpt)}”</p>`:''}`).join('');
  return `<aside class="zhihu-echo" aria-label="${title}"><div class="zhihu-echo-heading"><span class="zhihu-badge" aria-hidden="true">知</span><b>${title}</b></div><p>${esc(grounding?.echo??firstSentence(source.idea))}</p><details class="zhihu-echo-context"><summary>原文与适用条件 ↗</summary><small>${status}</small>${conditions}${references}<p>${esc(grounding?.boundary??source.scope)} 这是相关讨论的整理，不是原文逐字引述，也不是本局事件实录。</p></details></aside>`;
}
...
export function journeySourceBookMarkup(history=[]){
  ...
  return `<details class="journey-source-book"><summary><span>本局知乎经验册</span><b>${sources.length} 条独立来源</b></summary><p class="source-book-note">只收录本局已经历事件的相关讨论，同一原文只计一次；一条来源可供多个事件参考，不是每格一篇独立案例，也不是作者经历的复现。多数来源仅核对搜索片段，具体证据程度见各条。</p><div class="source-book-list">${sources.map(source=>`<article class="source-book-entry" data-source-id="${esc(source.id)}"><span class="tiny-label">${esc(source.scope)}</span><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(source.title)} ↗</a><small>${esc(source.sourceKind==='question'?'问题页 · '+source.author:'知乎作者 · '+source.author)}</small><p>${esc(source.idea)}</p><small>${esc(source.evidence??'CLI 搜索片段，未读取完整原文')}</small><div class="source-book-pages">与你经历的「${source.events.map(event=>esc(event.title)).join('」「')}」主题相关</div></article>`).join('')}</div></details>`;
}
