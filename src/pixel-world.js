...
export class PixelWorld {
  ...
  bindPointer(){
    ...
    this.pointerMove=e=>{if(!this.drag||!this.interactive)return;const dx=e.clientX-this.drag.x,dy=e.clientY-this.drag.y;if(Math.hypot(dx,dy)>5)this.drag.moved=true;const ratio=this.width/(this.canvas.clientWidth||1)/this.camera.scale;this.pan.x=this.drag.pan.x-dx*ratio;this.pan.y=this.drag.pan.y-dy*ratio;this.canvas.style.cursor=this.drag.moved?'grabbing':'grab';};
