...
const welcomeTalents=[
  ['defense','稳健防守型','带一份保障出发','开局带【留痕备忘录】，首次甩锅抵挡事件本身的消耗。','提前使用会消耗这次保护；累积疲惫仍需休息。'],
  ['ambitious','锐意进取型','成长更快，也更累','事件专业收益 +20%，负面事件情绪消耗 +10%。','更快成长，也更容易感到疲惫。'],
  ['optimistic','乐天知命型','换季时恢复情绪','每跨入一个新季节，自动恢复 15 点情绪。','随当次事件结算，不超过情绪上限。'],
];
export function talentDetailsMarkup(){return `<section class="talent-info"><h2>带上一种心态</h2><div class="talent-info-grid">${welcomeTalents.map(([,name,,effect,note])=>`<article><h3>${esc(name)}</h3><p>${esc(effect)}</p><small>${esc(note)}</small></article>`).join('')}</div></section>`;}
export function welcomeMarkup(setup,saved){return `<div class="welcome-top"><span class="tiny-label">知乎四时</span><span class="edition" aria-label="3D 田园版 · v4">四季田园</span></div>
  ...
  <div class="welcome-setup"><div class="welcome-settings-scroll" tabindex="0" role="region" aria-label="昵称和开局心态"><div class="welcome-name-field"><label class="name-label" for="character-name">怎么称呼你？</label><input id="character-name" maxlength="16" value="${esc(setup.name??'')}" autocomplete="off" placeholder="昵称（选填）"/></div><fieldset class="talent-picker"><legend>带上一种心态</legend><div class="talent-options">${welcomeTalents.map(([v,n,brief,d,note])=>`<label><input type="radio" name="talent" value="${v}" ${setup.talent===v?'checked':''} aria-describedby="talent-effect-${v}"/><span><b>${n}</b><small class="talent-brief">${brief}</small><small id="talent-effect-${v}" class="talent-effect">${d}</small><small class="talent-note">${note}</small></span></label>`).join('')}</div></fieldset><div class="welcome-extras"><button type="button" id="talent-details" aria-haspopup="dialog">查看心态说明 ↗</button></div></div><div class="welcome-actions"><button id="start-full" class="primary">走进我的四季 <span>→</span><small>40 站 · 约 6–10 分钟</small></button></div><p class="welcome-fineprint">虚构人生，认真选择。</p></div></div>`;}
